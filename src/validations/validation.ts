...
export class ValidationMessages {
  ...
  public static isPhoneNumberValid(phone: string) {
    return new Promise((resolve, reject) => {
      let phoneNumber = parsePhoneNumber(phone ? phone.toString() : "", "FR");

      if (!phoneNumber.isValid()) {
        reject({
          message: "Must be an correct phone number.",
          errorCode: 1,
        });
      } else {
        if (
          !EnvironmentConfigs.getAllowedCountryCodes().includes(
            phoneNumber.country as string
          )
        ) {
          reject({
            message: "Must be a France phone number.",
            errorCode: 1,
          });
        }
        resolve(true);
